...
def drain_queue_nowait(q: asyncio.Queue) -> list:
    items = []
    while not q.empty():
        instrument_id, blob, stale = q.get_nowait()
        decoded = msgpack.unpackb(blob, raw=False)
        items.append((instrument_id, decoded, stale))
    return items
...
def test_is_valid_delta_requires_all_fields():
    good = make_delta(seq=1)
    assert is_valid_delta(good) is True

    for field in ("side", "level", "price", "qty", "seq"):
        broken = make_delta(seq=1)
        del broken[field]
        assert is_valid_delta(broken) is False, f"missing '{field}' should be invalid"

    assert is_valid_delta({"type": "ack"}) is False
...
@pytest.mark.asyncio
async def test_resync_mode_pushes_stale_true_before_awaiting_snapshot():
    book = OrderBook("NIFTY")
    transport = FakeTransport()
    transport.snapshot_delay = 0.2
    transport.set_snapshot({"bids": [(100.0, 1.0)], "asks": [(100.5, 1.0)], "seq": 10})
    health = ConnectionHealth()
    redis_queue = asyncio.Queue()

    task = asyncio.create_task(
        resync_mode(transport, book, health, redis_queue, "NIFTY", triggering_msg=make_delta(seq=5))
    )

    await asyncio.sleep(0.02)
    assert not task.done()

    items = drain_queue_nowait(redis_queue)
    assert len(items) == 1, "expected exactly one push (STALE=True) before snapshot arrives"
    instrument_id, decoded, stale = items[0]
    assert instrument_id == "NIFTY"
...
@pytest.mark.asyncio
async def test_resync_mode_pushes_stale_false_with_fresh_book_on_success():
    book = OrderBook("NIFTY")
    transport = FakeTransport()
    transport.set_snapshot({"bids": [(100.0, 10.0)], "asks": [(100.5, 10.0)], "seq": 10})
    health = ConnectionHealth()
    redis_queue = asyncio.Queue()

    await resync_mode(transport, book, health, redis_queue, "NIFTY", triggering_msg=make_delta(seq=12))

    items = drain_queue_nowait(redis_queue)
    assert len(items) == 2, "expected STALE=True then STALE=False"
    assert items[0][2] is True
    assert items[1][2] is False
    assert items[1][1]["seq"] == 12
...
async def test_resync_mode_seeds_from_snapshot_and_replays_only_newer_deltas():
    ...
    last_seq = await resync_mode(transport, book, health, redis_queue, "NIFTY", triggering_msg=triggering_msg)
...
async def test_resync_mode_times_out_and_raises_connection_closed():
    ...
    with pytest.raises(ConnectionClosed):
        await resync_mode(
            transport, book, health, redis_queue, "NIFTY",
            triggering_msg=make_delta(seq=1), timeout=0.1,
        )
...
@pytest.mark.asyncio
async def test_heartbeat_raises_connection_closed_when_send_hangs():
    transport = FakeTransport()
    transport.send_hangs = True
    health = ConnectionHealth()

    with pytest.raises(ConnectionClosed, match="send\\(\\) did not complete"):
        await heartbeat_loop(transport, health, interval=5.0, send_timeout=0.05)
...
@pytest.mark.asyncio
async def test_delta_loop_survives_malformed_delta_by_forcing_resync():
    book = OrderBook("NIFTY")
    book.seq = 1
    transport = FakeTransport()
    transport.set_snapshot({"bids": [(77.0, 1.0)], "asks": [(77.5, 1.0)], "seq": 50})

    malformed = make_delta(seq=2)
    del malformed["price"]
    transport.queue_messages([malformed])
    ...
    assert book.seq == 50
    assert book.get_level("bid", 0) == (77.0, 1.0)


@pytest.mark.asyncio
async def test_resync_mode_drops_malformed_deltas_encountered_during_buffering():
    book = OrderBook("NIFTY")
    transport = FakeTransport()
    transport.snapshot_delay = 0.1
    transport.set_snapshot({"bids": [(1.0, 1.0)], "asks": [(1.0, 1.0)], "seq": 5})
    health = ConnectionHealth()
    redis_queue = asyncio.Queue()

    malformed = make_delta(seq=6)
    del malformed["qty"]
    transport.queue_messages([malformed, make_delta(seq=7, side="bid", level=4, price=55.0, qty=2.0)])

    last_seq = await resync_mode(
        transport, book, health, redis_queue, "NIFTY", triggering_msg=make_delta(seq=4)
    )

    assert book.get_level("bid", 4) == (55.0, 2.0)
    assert last_seq == 7